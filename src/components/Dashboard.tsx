              ...
              return (
                <div key={alert.budget.id} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    {IconComponent && <IconComponent size={16} className="text-yellow-600" />}
                    <span className="text-sm text-yellow-800">{alert.category?.name}</span>
                  </div>
                  <div className="text-right">
                    <span className={`text-sm font-medium ${
                      alert.isOverLimit ? 'text-red-600' : 'text-yellow-600'
                    }`}>
                      ₹{alert.spent.toLocaleString('en-IN')} / ₹{alert.budget.monthlyLimit.toLocaleString('en-IN')}
                    </span>
                    <span className={`text-xs ml-2 ${
                      alert.isOverLimit ? 'text-red-500' : 'text-yellow-500'
                    }`}>
